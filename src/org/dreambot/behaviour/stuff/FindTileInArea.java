...
public class FindTileInArea extends Leaf<Main> {
    ...
    @Override
    public int onLoop() {
    	Tile p = Map.getWalkable(API.randArea.getRandomTile());
		if(!API.randArea.contains(p))
		{
			for(int i = 0; i< 99; i++)
			{
				p = Map.getWalkable(API.randArea.getRandomTile());
				if(API.randArea.contains(p)) break;
			}
		}
		if(!API.randArea.contains(p))
		{
